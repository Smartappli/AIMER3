...
class ForgetPasswordView(AuthView):
    ...
    async def post(self, request):
        if request.method == "POST":
            ...
            if not user:
                await sync_to_async(messages.error)(request, "No user with this email exists.")
            ...
            if settings.EMAIL_HOST_USER and settings.EMAIL_HOST_PASSWORD:
                await sync_to_async(messages.success)(request, "A password reset email has been sent. Please check your inbox")
            else:
