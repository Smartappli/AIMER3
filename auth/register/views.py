...
class RegisterView(AuthView):
    ...
    async def post(self, request):
        ...
        if await User.objects.filter(username=username, email=email).aexists():
            await sync_to_async(messages.error)(request, "User already exists, Try logging in.")
            return redirect("register")
        elif await User.objects.filter(email=email).aexists():
            await sync_to_async(messages.error)(request, "Email already exists.")
            return redirect("register")
        elif await User.objects.filter(username=username).aexists():
            await sync_to_async(messages.error)(request, "Username already exists.")
            return redirect("register")

        # Create the user and set their password
        created_user = await User.objects.acreate_user(username=username, email=email, password=password)
        ...
        if settings.EMAIL_HOST_USER and settings.EMAIL_HOST_PASSWORD:
            await sync_to_async(messages.success)(request, "Verification email sent successfully")
        else:
            await sync_to_async(messages.error)(request, "Email settings are not configured. Unable to send verification email.")
